...
do
	...
	if [ ! -f prot_merged.fasta ]; then
        ...
        echo "java -Xmx2g -jar ${JAR_DIR}/ReadSeq.jar rm-dupseq -d -i prot_merged.fasta -o prot_merged_derep_rmdup.fasta"
	java -Xmx${MAX_JVM_HEAP} -jar ${JAR_DIR}/Clustering.jar derep -o temp_prot_derep.fa  ids samples prot_merged.fasta || { echo "get unique contigs failed for ${gene}" ; continue; }
        java -Xmx${MAX_JVM_HEAP} -jar ${JAR_DIR}/ReadSeq.jar rm-dupseq -d -i temp_prot_derep.fa -o prot_merged_derep_rmdup.fasta || { echo "get unique contigs failed for ${gene}" ; continue; }
	...
	${HMMALIGN} -o alignment/aligned.stk ${REF_DIR}/${gene}/originaldata/${gene}.hmm ../prot_merged_derep_rmdup.fasta || { echo "hmmalign failed" ;  continue; }
	...
	## if there is no overlap between the contigs, mcClust will throw errors, we should use the ../prot_merged_derep_rmdup.fasta as  prot_rep_seqs.fasta 
	java -Xmx2g -jar ${JAR_DIR}/Clustering.jar dmatrix  -c 0.5 -I derep.fa -i ids -l 25 -o dmatrix.bin || { echo "dmatrix failed" ; cp ../prot_merged_derep_rmdup.fasta prot_rep_seqs.fasta ; }
	...
	if [ -f dmatrix.bin ]; then
		java -Xmx2g -jar ${JAR_DIR}/Clustering.jar cluster -d dmatrix.bin -i ids -s samples -o complete.clust || { echo "cluster failed" ;  exit 1; }
        	...
        	java -Xmx2g -jar ${JAR_DIR}/Clustering.jar to-unaligned-fasta complete.clust_rep_seqs.fasta > prot_rep_seqs.fasta || { echo " to-unaligned-fasta failed" ;  exit 1; }
        fi


	grep '>' prot_rep_seqs.fasta |cut -f1 | cut -f1 -d ' ' | sed -e 's/>//' > id || { echo " failed" ;  exit 1; }
	java -Xmx2g -jar ${JAR_DIR}/Clustering.jar filter-seqs id ../nucl_merged.fasta false > nucl_rep_seqs.fasta || { echo " filter-seqs failed" ;  exit 1; }

	rm -r derep.fa dmatrix.bin nonoverlapping.bin alignment samples ids complete.clust_rep_seqs.fasta id

	# remove chimeras
        ${UCHIME} --input nucl_rep_seqs.fasta --db ${REF_DIR}/${gene}/originaldata/nucl.fa --uchimeout results.uchime.txt -uchimealns result_uchimealn.txt || { echo "chimera check failed" ;  continue; }
        egrep '\?$|Y$' results.uchime.txt | cut -f2 | cut -f1 -d ' ' | cut -f1 > chimera.id || { echo " egrep failed" ;  exit 1; }
        java -Xmx2g -jar ${JAR_DIR}/Clustering.jar filter-seqs chimera.id nucl_rep_seqs.fasta true > nucl_rep_seqs_rmchimera.fasta || { echo " filter-seqs failed" ;  exit 1; }
        ...
        java -jar ${JAR_DIR}/FrameBot.jar framebot -N -l ${MIN_LENGTH} -o ${gene}_${K_SIZE} ${REF_DIR}/${gene}/originaldata/framebot.fa nucl_rep_seqs_rmchimera.fasta || { echo "FrameBot failed for ${gene}" ; continue; }

	## or find the closest matches of protein representatives using AlignmentTool pairwise-knn

	## find kmer coverage of the representative seqs, this step takes time, recommend to run multiplethreads
	echo "### Kmer abundance"
        echo "java -Xmx2g -jar ${JAR_DIR}/KmerFilter.jar kmer_coverage -t ${THREADS} -m match_reads.fa ${K_SIZE} nucl_rep_seqs_rmchimera.fasta ${gene}_coverage.txt ${gene}_abundance.txt ${SEQFILE}"
        java -Xmx2g -jar ${JAR_DIR}/KmerFilter.jar kmer_coverage -t ${THREADS} -m match_reads.fa ${K_SIZE} nucl_rep_seqs_rmchimera.fasta ${gene}_coverage.txt ${gene}_abundance.txt ${SEQFILE} || { echo "kmer_coverage failed" ;  continue; }
