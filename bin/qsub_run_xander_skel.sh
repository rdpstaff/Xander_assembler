...
source $BASEDIR/qsub_xander_setenv.sh
...
$BASEDIR/run_xander_build.sh $BASEDIR/qsub_xander_setenv.sh

## find starting kmers, multiple genes should be run together to save time, has multithread option
$BASEDIR/run_xander_findStarts.sh $BASEDIR/qsub_xander_setenv.sh
...
for gene in ${genes[*]}
do
	echo "$BASEDIR/run_xander_search.sh $BASEDIR/qsub_xander_setenv.sh ${gene}"
	$BASEDIR/run_xander_search.sh $BASEDIR/qsub_xander_setenv.sh ${gene}  
done
